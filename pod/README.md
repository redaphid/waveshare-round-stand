...
![pod, three-quarter](renders/01-pod-gap26-front-quarter.png)

## What you print

| part | qty | file | notes |
|---|---|---|---|
| pod | 1 | `stl/pod-gap26.stl` **or** `stl/pod-gap12.stl` | see gap choice below |
...
## The gap choice — pick by your USB-C cable

The 1.46's port is on its bottom edge and fires radially. Whatever hangs from it
lands in the gap below that cup.

| | `pod-gap26` | `pod-gap12` |
|---|---|---|
| cable | **straight** plug (~24 mm body) | **right-angle** plug |
| rim-to-rim gap | 26 mm | 12 mm |
| height | 209 mm | 181 mm |
| volume | 93 cm³ | 82 cm³ |

![26 vs 12](renders/01-pod-gap12-front-quarter.png)

Right-angle cables make the tighter, better-proportioned pod. Both fit the
Centauri Carbon 2 bed (256 mm).
...
The port's plug drops through a slot in the bottom of each cup into the gap
below. From there the leads run into the **wiring channel down the back of the
spine** (open to the rear, hidden from you) and out the bottom.
...
- Everything is generated by `build_pod.py` (manifold3d + trimesh, in the
  ...
  number, re-run: `~/.venvs/cad/bin/python build_pod.py 26` and `… 12`.
