...
import sys
groups = {}
for key, p in bs.BOARDS.items():
    groups.setdefault(p.get("group", "stand"), []).append((key, p))

for gname, members in groups.items():
  n = len(members)
  # tallest board top in this group sets the vertical extent
  ztop = 0
  for key, p in members:
    A, B, C, axis, perp, bc = bs.build_profiles(p)
    ztop = max(ztop, bc[1] + axis[1]*p["board_d"])
  fig, axes = plt.subplots(2, n, figsize=(5.4*n, 11.5), dpi=125, squeeze=False)

  for col, (key, p) in enumerate(members):
      A, B, C, axis, perp, bc = bs.build_profiles(p)
      ...
      ax.set_xlim(-12, 56); ax.set_ylim(-4, ztop + 8); ax.set_aspect("equal")
  ...
  title = {"stand": "low stands — battery, right-angle lead, or side-mounted port",
           "dock":  "docks — straight USB-C plug hanging from the bottom port"}[gname]
  fig.suptitle(f"Waveshare round-board {title}\nside sections (top) and front elevations (bottom)",
               fontsize=12)
  fig.tight_layout()
  fig.savefig(f"preview-{gname}s.png")
  print(f"wrote preview-{gname}s.png")
  plt.close(fig)
